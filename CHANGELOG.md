...
### Changed
- **Strategy shift**: stop guessing LinkedIn class names. Use ARIA roles
  + smart fallback chain.
  - Containers list adds `[role="article"]` first (ARIA, stable across
    LinkedIn redesigns).
  - When no specific container matches, fall back to `<main>` element
    (NOT `<body>`) — this still excludes sidebar/right rail/nav by HTML
    semantics so worst case we translate the main column only, never
    the whole page.
- forbidAncestors restored `[role="complementary"]` (LinkedIn's right rail).
